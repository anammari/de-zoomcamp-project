...
def next_month(bucketname):
   ...
   '''
    ...
    latest_year = max([int(os.path.basename(file)[:4]) for file in files])
    latest_year_files = [file for file in files if int(os.path.basename(file)[:4]) == latest_year]
    lastfile = os.path.basename(latest_year_files[-1])
